 ...
 * @access   public
 */
class Server extends NuSoap\Base
{
    /**
    ...
    function __construct($wsdl = false)
    {
        ...
        if (isset($debug)) {
            $this->debug("In Server, set debug_flag=$debug based on global flag");
            $this->debug_flag = $debug;
        } elseif (isset($_SERVER['QUERY_STRING'])) {
            $qs = explode('&', $_SERVER['QUERY_STRING']);
            foreach ($qs as $v) {
                if (substr($v, 0, 6) == 'debug=') {
                    $this->debug("In Server, set debug_flag=" . substr($v, 6) . " based on query string #1");
            ...
            foreach ($qs as $v) {
                if (substr($v, 0, 6) == 'debug=') {
                    $this->debug("In Server, set debug_flag=" . substr($v, 6) . " based on query string #2");
                    $this->debug_flag = substr($v, 6);
                }
            }
        }
        ...
        // wsdl
        if ($wsdl) {
            $this->debug("In Server, WSDL is specified");
    ...
    function serialize_return()
    {
        ...
        // if fault
        if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'NuSoap\soap_fault') || (get_class($this->methodreturn) == 'NuSoap\Fault'))) {
    ...
    function fault($faultcode, $faultstring, $faultactor = '', $faultdetail = '')
    {
        ...
        $this->fault = new NuSoap\Fault($faultcode, $faultactor, $faultstring, $faultdetail);
